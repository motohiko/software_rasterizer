...
{
    Vector4 TextureUnit::SamplePoint(const Sampler2D* sampler, int tx, int ty)
    {
    ...
    Vector4 TextureUnit::SampleNearestPoint(const Sampler2D* sampler, const Vector2& texcoord)
    {
        ...
        Vector4 color = SamplePoint(sampler, txi, tyi);
    ...
    Vector4 TextureUnit::SampleBilinear(const Sampler2D* sampler, const Vector2& texcoord)
    {
        ...
        // 補間対象のテクセルを取得
        Vector4 q00 = SamplePoint(sampler, xi + 0, yi + 0);
        Vector4 q01 = SamplePoint(sampler, xi + 1, yi + 0);
        Vector4 q10 = SamplePoint(sampler, xi + 0, yi + 1);
        Vector4 q11 = SamplePoint(sampler, xi + 1, yi + 1);
